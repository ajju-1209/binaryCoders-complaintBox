...
const getComplaintsWorker = (status) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(
            `/api/complaints/worker/get?status=${status.join("&status=")}`,
            config
        );
        dispatch({
    ...
    (status, department, complaintType) => async (dispatch, getState) => {
        try {
            ...
            if (
                ...
                Data = await axios.get("/api/users/admin/getUsers", config);
